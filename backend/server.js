...
async function createDocumentPdfBuffer({ title, textContent, htmlContent }) {
  const bodyText = String(textContent || '').trim() || htmlToPlainText(htmlContent);
  ...
  return new Promise((resolve, reject) => {
    ...
    pdf.font('Helvetica-Bold').fontSize(18).fillColor('#1e3a5f').text(String(title || 'Document'));
    pdf.moveDown(0.5);
    pdf.font('Helvetica').fontSize(10).fillColor('#64748b').text('Generated by RoleRocket AI');
    pdf.moveDown(1);
