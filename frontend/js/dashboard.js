  ...
  if (tipsEl) {
    try {
      ...
      if (isAdmin || isLifetime || isSubscribed) {
        ...
        tips = '<ul><li>Use the ATS Optimizer before every submission.</li><li>Use Interview Prep AI for upcoming interviews.</li></ul>';
      } else if (plan === 'elite') {
...
const PLAN_GUIDE_CONTENT = {
  ...
  premium: {
    ...
    bullets: [
      ...
      '1-Click Apply Queue shortens the jump from Prepared queue to submitted application.'
    ],
...
async function syncLifetimeOfferUi() {
  ...
  try {
    ...
    if (data?.offerActive) {
      lifetimeDashboardPriceEl.innerHTML = '<span class="price-original">$350</span> $299<span> one-time</span>';
    ...
    lifetimeDashboardPriceEl.innerHTML = '$350<span> one-time</span>';
    lifetimeOfferPillEl.textContent = '🔥 LIFETIME ACCESS';
    lifetimeDashboardOfferNoteEl.textContent = 'Limited offer sold out. Lifetime is now $350 one-time.';
  } catch (err) {
