...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function formatCareerPathForPdf(text, doc) {
    ...
    doc.setFontSize(16);
    doc.text('Career Path Simulation', 25, y);
    y += 24;
  ...
  if (pdfBtn) {
    pdfBtn.onclick = function() {
      ...
      if (!text) {
        output.innerHTML = '<div style="color:#dc2626;">No simulation to download.</div>';
        return;
      }
      if (!window.jspdf) {
      ...
      const doc = new jsPDF();
      formatCareerPathForPdf(text, doc);
      doc.save('career-path-simulation.pdf');
  ...
  if (wordBtn) {
    wordBtn.onclick = function() {
      ...
      const content = 'Career Path Simulation\n\n' + text;
      const blob = new Blob([content], { type: 'application/msword' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'career-path-simulation.doc';
      a.click();
