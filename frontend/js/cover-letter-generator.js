...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function startCreditCheckout(bundle) {
    ...
    if (isIosNativeApp) {
      output.innerHTML = '<div style="color:#dc2626;">Document credit purchases are not available in the iOS app build. Please use the web version.</div>';
      setBillingButtonsDisabled(false);
      return;
    }
    const token = getAuthToken();
  ...
  async function confirmDocumentCheckoutSession(sessionId) {
    ...
    try {
      ...
      // Silent fallback: status refresh still runs.
    }
  }
  ...
  async function loadResumeFileIntoField(file, textarea, messageEl) {
    ...
    try {
      if (file.type === 'application/pdf' || file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
        ...
        const res = await fetch(getApiEndpoint('/api/resume/upload'), {
          method: 'POST',
          headers,
          body: formData
        });
  ...
  generateBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      const headers = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const res = await fetch(getApiEndpoint('/api/cover-letter/generate'), {
        method: 'POST',
        headers,
